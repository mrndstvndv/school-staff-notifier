...
function createBaseIssueList(): IssueList {
  return { issues: [] };
}

export const IssueList: MessageFns<IssueList> = {
  encode(message: IssueList, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    for (const v of message.issues) {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): IssueList {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseIssueList();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
  ...
  create<I extends Exact<DeepPartial<IssueList>, I>>(base?: I): IssueList {
    return IssueList.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<IssueList>, I>>(object: I): IssueList {
    const message = createBaseIssueList();
